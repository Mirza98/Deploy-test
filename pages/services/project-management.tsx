...
const ProjectManagement: NextPage = () => {
  let { t } = useTranslation('project-management');
  const { projectManagment } = ServicesPageData;
  return (
    <>
      <Head>
      ...
      <Section container className="py-0">
        ...
        <Columns multiline className="py-0 my-4rem is-tablet">
          <Column className="is-6-tablet is-8-widescreen is-7-widescreen-only is-flex is-flex-direction-column is-justify-content-space-between">
            <p className="designing-block has-text-justified-tablet">{t('p1')}</p>

            <Box>
              <Columns>
                <Column>
                  <ul className="dashed">
                    {projectManagment.serviceDescriptionList.map((item, i) => {
                      return (
                        <li className="title is-4 mb-4" key={'Ip9rxPiriv' + i}>
                          {t(`projectManagment.serviceDescriptionList.${i}`)}
                        </li>
                      );
                    })}
                  </ul>
          ...
          <Column className="is-6-tablet is-4-widescreen is-5-widescreen-only has-text-centered-touch has-text-right-desktop">
            <img
              ...
              src="/slike/upravljanje-projektima.jpeg"
              alt="our team on project"></img>
          </Column>
        </Columns>
      </Section>
      <RelatedProjects typeOf="service" projectsIds={[8, 12]} tags={['upravljanje-projektima']} />
    </>
  );
};
